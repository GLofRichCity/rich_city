...
contract Traits is Ownable {
    ...
    function drawSVG(uint256 tokenId) public view returns (string memory) {
        ...
        uint8 shift = s1.kind * 10; //  10 20 30 40
        if (s1.kind == 1 && s1.level > 3) shift = 40;
        string memory svgString;
        if (s1.kind == 1 && s1.level <= 3) {
            svgString = string(
                abi.encodePacked(
                    drawTrait(
                        traitData[5 + shift][
    ...
    function tokenURI(uint256 tokenId) public view returns (string memory) {
        ...
        string memory metadata = string(
            abi.encodePacked(
                '{"name": "',
                s1.kind == 1
                    ? s1.level <= 3 ? "Gangster #" : "Gangster LEAD #"
                    : s1.kind == 2
                    ? "SWAT #"
                    : "EVA #",
                tokenId.toString(),

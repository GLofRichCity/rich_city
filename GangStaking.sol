...
interface IERC721 {
    ...
    function getGangTraits(uint256 tokenId)
        external
        view
        returns (GangTrait memory);
}
...
contract GangStaking is AccessControl, IERC721Receiver, Pausable {
    ...
    mapping(address => uint256) public leaderId;
    mapping(address => uint256) public userGang;
    mapping(address => bool) public isStaked;

    constructor(address _DRUG, uint256 _initRebaseStartTime) {
    ...
    function unstakeGang(uint256 tokenId) external whenNotPaused {
        require(msg.sender == packs[tokenId].owner, "now owner");
        require(
            packs[tokenId].lastTime + 7 days >= block.timestamp, // 7 days
            "Wait for release"
        );
        packs[tokenId].owner = address(0);
        packs[tokenId].enabled = false;
        delete leaderId[msg.sender];
    ...
    function _joinGang(uint256 tokenId, address _user) internal {
    ...
    function attack(uint256 tokenId, uint256 target) external whenNotPaused {
        ...
        require(packs[tokenId].restUntil <= block.timestamp, "Attack CD");
        if (packs[target].defense > 0) {
            packs[target].defense--;
        } else {
            // decrease target score
            packs[target].attackeds++;
            packs[target].score = packs[target].score.mul(remainingP.div(100));
        }
        ...
        emit Attack(
            ...
            target,
            block.timestamp,
            Gang.getGangTraits(tokenId).name,
            Gang.getGangTraits(target).name
        );
    }
    ...
    function _sendReward() internal returns (bool) {
        if (block.timestamp >= initRebaseStartTime + round * 1 days) {
            //1 days
            loadScore();
            sortByScore();
            ...
            if (amount > totalAmount) {
                DRUG.safeTransfer(burnAddress, amount.sub(totalAmount));
            }
            round++;
            if (autoRebase) {
                return _sendReward();
        ...
        for (uint256 i = 0; i < gangs.length; i++) {
            ...
            if (packs[gangs[i]].enabled) {
                ...
                uint256 expected = round / 7 + 1 >= 10
                    ? 240000 ether
                    : (round / 7 + 1) * 24000 ether; // 24000 ether 24000 ether

                if (DRUGAmount > packs[gangs[i]].drugAmount) {
                    if (
                        DRUGAmount.sub(packs[gangs[i]].drugAmount) >= expected
                    ) {
                        packs[gangs[i]].attackCount +=
                            DRUGAmount.sub(packs[gangs[i]].drugAmount) /
                            expected;
                    }
                }

                if (CASHAmount > packs[gangs[i]].cashAmount) {
                    if (
                        CASHAmount.sub(packs[gangs[i]].cashAmount) >=
                        expected * 2
                    ) {
                        uint256 count = CASHAmount.sub(
                            packs[gangs[i]].cashAmount
                        ) / (expected * 2);
                        packs[gangs[i]].defense += count;
                        emit BuildDefense(
                            gangs[i],
                            block.timestamp,
                            count,
                            Gang.getGangTraits(gangs[i]).name
                        );
                    }
                }
    ...
    function sendToken(uint256 totalAmount) internal {
        ...
        if (totalAmount > 0) {
            ...
            if (n) {
                ...
                DRUG.safeTransfer(burnAddress, sendAmount);
            }
            if (c) {
    ...
    function getRebaseAmount() public view returns (uint256 rebaseAmount) {
        ...
        if (block.timestamp < initRebaseStartTime) {
            rebaseAmount = maxAmount.mul(1776700000).div(1e10).div(30);
        } else {
            uint256 deltaTimeFromInit = block.timestamp.sub(
                initRebaseStartTime
            );
    ...
    function setGang(address _Gang) external onlyRole(DEFAULT_ADMIN_ROLE) {
        Gang = IERC721(_Gang);
    }

    function setAutoRebase(bool _a) external onlyRole(DEFAULT_ADMIN_ROLE) {
        autoRebase = _a;
    }

    function setRemainingP(uint256 _remainingP)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        remainingP = _remainingP;
    }
    ...
    function getNameAndLink(uint256 tokenId)
        public
        view
        returns (string memory s)
    {
        s = Gang.getGangTraits(tokenId).name;
    }
